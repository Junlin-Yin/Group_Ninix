...
struct prio_array *expired;  //过期队列指针
struct prio_array arrays[2];  //活动队列和过期队列
unsigned int timestamp_last_tick = 0;
unsigned int array_exchange_count = 0;


// int best_expired_prio;  //过期队列中静态优先级最高的进程
// unsigned int expired_timestamp; //过期队列中的最老进程插入的时间
// struct mm_struct *prev_mm;  //存放被替换进程的进程描述符指针
...
//根据fork时指定的优先级决定内核线程的入口函数
//如果是实时进程 因为优先级高 所以会获得较多的时间片
//因此将实时进程的入口函数设为runprog2 即有exit
if (0 < set_prio  && set_prio <= MAX_RT_PRIO / 2)
{
kernel_printf("rt task\n");
return do_fork(task_name, (void*)runprog2, 0, (void *)0, set_prio);
...
//如果是普通进程 获得的时间片较少
//将入口函数设置为runprog1 即内部为一个死循环 方便调度调试
else if (set_prio > MAX_RT_PRIO)
{
kernel_printf("normal task\n");
return do_fork(task_name, (void*)runprog1, 0, (void *)0, set_prio);
...
p->timestamp = sched_clock();   //进程创建的时间戳
}
...
kernel_printf("running program3\t current_task: %d\n", current_task->pid);
if (current_task->pid == 0)
return 0;
}
}
