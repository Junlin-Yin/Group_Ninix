...
enum insertion_state { FAILED, NON_CONNECTION, FORWARD_CONNECTION, BACKWARD_CONNECTION, TWO_WAY_CONNECTION, AFTER_LAST_CONNECTION, FIRST_INSERTION };
...
struct bootmm {   //store infos of the whole bootmms, it is an entirety
	...
	unsigned char* e_map;
	unsigned int last_alloc_end; //last allocate page end(page number)
...
extern enum insertion_state insert_mminfo(struct bootmm* mm, unsigned int start, unsigned int end, unsigned int type);
...
extern void show_bootmap_info(unsigned char* msg);

extern void show_bootmm();
